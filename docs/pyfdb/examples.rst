...
.. code-block:: python
    ...
    mars_selection = {
        ...
        "key-2": "value-2/to/value-4/by/value-5",       # Range statement
        "key-3": ["0.1", 0.2, "0.5"],                   # Mixed types
        "key-4": [0.1, 0.2, 0.5],                       # Float values
        "key-5": [1, 2, 0.5],                           # Integer and float values
        "key-6": 1,                                     # Single int value
        "key-7": [1 + 0.5 * x for x in range(2)],       # List of float value generated by a list expression
    }
...
.. invisible-code-block: python
   ...
   assert len(internal_mars_selection) == 7
   assert "key-1" in internal_mars_selection
   assert internal_mars_selection["key-1"] == "value-2/value-4/value-5"
   assert "key-2" in internal_mars_selection
   assert internal_mars_selection["key-2"] == "value-2/to/value-4/by/value-5"
   assert "key-3" in internal_mars_selection
   assert internal_mars_selection["key-3"] == "0.1/0.2/0.5"
   assert "key-4" in internal_mars_selection
   assert internal_mars_selection["key-4"] == "0.1/0.2/0.5"
   assert "key-5" in internal_mars_selection
   assert internal_mars_selection["key-5"] == "1/2/0.5"
   assert "key-6" in internal_mars_selection
   assert internal_mars_selection["key-6"] == "1"
   assert "key-7" in internal_mars_selection
   assert internal_mars_selection["key-7"] == "1.0/1.5"
...
the step loop, therefore causing a flush of the archived data. In case the
:ref:`archive` calls fail, an error would end the iteration. Make sure to
handle potential error in those scenarios accordingly. The correct order of
operations ultimately depends on the individual use cases.
...
.. note::
   ...
   you can see that the param key contains a list of values. Contrary to an
   Identifier, which is a fully specified ``MarsSelection`` and therefore only
   contains singular values, even ``x/to/y/by/z`` ranges are allowed. We recommend to
   use list of values at all given times, as seen in :ref:`mars_selection_label`.
