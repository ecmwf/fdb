...
void LegacyKey::set(const std::string& keyword, const std::string& value)
{
    StringDict::value_type p = translate(keyword, value);
    key_.set(p.first, p.second);
    if(p.second == "off") {
        key_.unset(p.first);
    }
}

StringDict::value_type LegacyKey::translate(const std::string& key, const std::string& value)
{
