...
void DaosCatalogueWriter::archive(const Key& idxKey, const Key& datumKey, std::unique_ptr<FieldLocation> fieldLocation) {
    // const Key& key, std::unique_ptr<FieldLocation> fieldLocation) {
    ...
    for (Key::const_iterator i = datumKey.begin(); i != datumKey.end(); ++i) {
        ...
        std::string value = datumKey.canonicalValue(keyword);
...
static fdb5::CatalogueWriterBuilder<fdb5::DaosCatalogueWriter> builder("daos.writer");
