...
PMemDBWriter::PMemDBWriter(const CanonicalKey& key, const eckit::Configuration& config) :
    PMemDB(key, config) {}
...
bool PMemDBWriter::selectIndex(const CanonicalKey& idxKey) {
...
void PMemDBWriter::archive(const CanonicalKey& key, const void *data, Length length) {
