...
Client::EndpointList storeEndpoints(const Config& config) {
    ...
    // endpoints used in RemoteFieldLocations can differ from the store endpoints (e.g. different networs; caninical
    // store names) if so, the canonical names must be provided in the fieldLocationEndpoints config, otherwhise the
...
eckit::DataHandle* RemoteStore::dataHandle(const FieldLocation& fieldLocation, const std::string& tracingID,
    ...
    std::cout << "RemoteStore::dataHandle - fieldLocation: " << fieldLocation.fullUri() << ", remapKey: " << remapKey
              << ", tracingID: " << tracingID << std::endl;
