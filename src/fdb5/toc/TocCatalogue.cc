...
MoveVisitor* TocCatalogue::moveVisitor(const Store& store, const metkit::mars::MarsRequest& request, const eckit::URI& dest, bool removeSrc, int removeDelay, eckit::Transport& transport) const {
    return new TocMoveVisitor(*this, store, request, dest, removeSrc, removeDelay, transport);
}
