...
private:  // methods
    std::set<eckit::PathName> databases(const std::set<CanonicalKey>& keys, const std::vector<eckit::PathName>& dirs,
                                        const Config& config) const;

    std::vector<eckit::URI> databases(const CanonicalKey& key, const std::vector<eckit::PathName>& dirs, const Config& config) const;
...
protected: // methods
    ...
    virtual eckit::URI location(const CanonicalKey& key, const Config& config) const override;

    virtual bool canHandle(const eckit::URI&, const Config& config) const override;

    // virtual std::vector<eckit::URI> allLocations(const CanonicalKey& key, const Config& config) const override;
    ...
    virtual std::vector<eckit::URI> writableLocations(const CanonicalKey& key, const Config& config) const override;
