...
void TocHandler::writeInitRecord(const Key& key) {
    ...
    if (len == 0) {
        ...
        std::unique_ptr<TocRecord> r2(
            new TocRecord(serialisationVersion_.used(), TocRecord::TOC_INIT));  // allocate TocRecord on heap (MARS-779)
...
void TocHandler::writeClearRecord(const Index& index) {
    ...
    std::unique_ptr<TocRecord> r(
        new TocRecord(serialisationVersion_.used(),
                      TocRecord::TOC_CLEAR));  // allocate (large) TocRecord on heap not stack (MARS-779)
...
void TocHandler::writeIndexRecord(const Index& index) {
    ...
    struct WriteToStream : public IndexLocationVisitor {
        ...
        virtual void operator()(const IndexLocation& l) {
            ...
            std::unique_ptr<TocRecord> r(
                new TocRecord(handler_.serialisationVersion_.used(),
