...
public:
    explicit TocStatsTimer(const std::string& label, eckit::Timer& t, std::function<void(const std::string&, eckit::Timer&)> fn) : 
...
class TocIOStats : public eckit::NonCopyable {

public:

    TocIOStats(const std::string& prefix = std::string());
    ~TocIOStats();
