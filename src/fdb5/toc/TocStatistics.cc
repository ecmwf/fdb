...
TocStatistics& TocStatistics::operator+=(const TocStatistics &rhs) {
...
void TocStatistics::add(const DbStatistics& rhs)
{
    const TocStatistics& stats = dynamic_cast<const TocStatistics&>(rhs);
    *this += stats;
}


void TocStatistics::report(std::ostream &out, const char *indent) const {
