...
public:
    ...
    FDBHammer(int argc, char** argv) :
        fdb5::FDBTool(argc, argv), verbose_(false), md_check_(false), full_check_(false) {
        ...
        options_.push_back(new eckit::option::SimpleOption<bool>(
            "full-check",
            "Insert a metadata checksum (checksum of the field key + unique operation identifier) and a checksum of "
            "the full "
            "field data payload, on every field write, at the begginning of the encoded message data payload. The data "
            "checksum "
            "includes the unique operation identifier in its calculation, and the resulting checksum is written at the "
            "beginning "
            "of the encoded message data payload between the field key checksum and the operation identifier. "
            "On every field read, the key checksum is extracted from the message data payload, the key checksum is "
            "recalculated "
            "from FDB field key, and both checksums are compared. The checksum of the data payload is also extracted "
            "and "
            "recalculated from the message data payload (excluding the key checksum but including the operation "
            "identifier), "
        ...
        options_.push_back(
            new eckit::option::SimpleOption<long>("step-window",
                                                  "Number of seconds allocated to perform I/O for a step, and slept if "
                                                  "not consumed, for writers in ITT mode. Defaults to 10."));
        options_.push_back(
        ...
        options_.push_back(
            new eckit::option::SimpleOption<long>("ppn",
                                                  "Number of processes per node the benchmark is being run on. "
...
void barrier_internode(std::vector<std::string>& nodes, int& port, int& max_wait) {
    ...
    if (hostname == nodes[0]) {
        ...
        if (timeout == 0)
            throw eckit::TimeOut("Exceeded time limit waiting for connection to barrier leader socket.", max_wait);
                ...
                for (size_t param = 0; param < nparams; ++param) {
                    ...
                    if (full_check_ or md_check_) {
                        ...
                        if (md_check_) {
                            ...
                            // message = grib header | fdb key checksum | unique id checksum | grib data | fdb key
                            // checksum | unique id checksum
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData], &key_hi, sizeof(key_hi));
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + sizeof(key_hi)], &key_lo,
                                     sizeof(key_lo));
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + 2 * sizeof(key_lo)], &uid_hi,
                                     sizeof(uid_hi));
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + 3 * sizeof(key_lo)], &uid_lo,
                                     sizeof(uid_lo));
                            ::memcpy(&const_cast<char*>(buffer)[offsetAfterData - 4 * sizeof(key_lo)], &key_hi,
                                     sizeof(key_hi));
                            ::memcpy(&const_cast<char*>(buffer)[offsetAfterData - 3 * sizeof(key_lo)], &key_lo,
                                     sizeof(key_lo));
                            ::memcpy(&const_cast<char*>(buffer)[offsetAfterData - 2 * sizeof(key_lo)], &uid_hi,
                                     sizeof(uid_hi));
                            ::memcpy(&const_cast<char*>(buffer)[offsetAfterData - 1 * sizeof(key_lo)], &uid_lo,
                                     sizeof(uid_lo));
                        }
                        ...
                        if (full_check_) {
                            ...
                            ::memcpy(&const_cast<char*>(buffer)[uidChecksumOffset + sizeof(key_lo)], &uid_lo,
                                     sizeof(uid_lo));
                            ...
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + sizeof(key_hi)], &key_lo,
                                     sizeof(key_lo));
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + 2 * sizeof(key_lo)], &data_hi,
                                     sizeof(data_hi));
                            ::memcpy(&const_cast<char*>(buffer)[offsetBeforeData + 3 * sizeof(key_lo)], &data_lo,
                                     sizeof(data_lo));
                        }
                    }
...
void FDBHammer::executeRead(const eckit::option::CmdArgs& args) {
    ...
    if (itt_ && uri_file.empty()) {
        ...
        if (start_at > 0 && stop_at < nlevels * nparams - 1 && first_level == last_level) {
            ...
            std::vector<std::string> paramlist_body(paramlist.begin() + (start_at % nparams),
                                                    paramlist.begin() + (stop_at % nparams) + 1);
