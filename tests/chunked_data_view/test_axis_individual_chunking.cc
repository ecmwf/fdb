...
CASE("RequestManipulation | Individual Chunking | Axis test single axis for Indices | Can create a subindex") {
    ...
    for (size_t i = 0; i < axis.parameters().size(); ++i) {
        ...
        CASE("index_mapping | 2 axes 2/1 param | Chunked | Valid access") {
            ...
            const std::vector<chunked_data_view::Axis> axes = {{{date_parameter, time_parameter}}, {{steps_parameter}}};
        ...
        CASE("index_mapping | 3 axes 2/1 param | Chunked/Non-Chunked| Valid access") {
            ...
            const std::vector<chunked_data_view::Axis> axes = {
                {{date_parameter, time_parameter}}, {{steps_parameter}}, {{params_parameter}}};
        ...
        CASE("index_mapping | axis_index_to_buffer_index | extension axis remaps stride and offset") {
            ...
            const std::vector<chunked_data_view::Axis> axes = {{{a_param}}, {{b_param}}, {{c_param}}};
...
//
//                     auto request_copy = request;
//                     auto chunk_index = l + k * fourth_values.size() + j * (fourth_values.size() *
//                     third_values.size()) +
...
//
//         for (size_t i = 0;
//              i < first.values().size() * second.values().size() * third.values().size() * fourth.values().size();
//              ++i) {
//             expected_chunks.emplace_back(1);
//         }
//
