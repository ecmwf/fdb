...
CASE("archives_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().flush, 0);
    EXPECT_EQUAL(spy2.counts().flush, 0);
    EXPECT_EQUAL(spy3.counts().flush, 0);
    ...
    for (int f = 0; f < nflush; f++) {
        for (int a = 0; a < narch; a++) {
            ...
            EXPECT_EQUAL((spy1.counts().archive + spy2.counts().archive + spy3.counts().archive),
                         (1 + a + (narch * f)));
            EXPECT_EQUAL(spy1.counts().flush + spy2.counts().flush + spy3.counts().flush, flush_count);
        }
...
CASE("retrieves_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().inspect, 1);
    EXPECT_EQUAL(spy2.counts().inspect, 1);
    EXPECT_EQUAL(spy3.counts().inspect, 1);

    req.setValuesTyped(new metkit::mars::TypeAny("class"), std::vector<std::string>{std::string("rd")});
    fdb.inspect(req);

    EXPECT_EQUAL(spy1.counts().inspect, 2);
    EXPECT_EQUAL(spy2.counts().inspect, 2);
    EXPECT_EQUAL(spy3.counts().inspect, 2);
    ...
    EXPECT_EQUAL(spy1.counts().inspect, 3);
    EXPECT_EQUAL(spy2.counts().inspect, 3);
    EXPECT_EQUAL(spy3.counts().inspect, 3);
    ...
    EXPECT_EQUAL(spy1.counts().inspect, 4);
    EXPECT_EQUAL(spy2.counts().inspect, 4);
    EXPECT_EQUAL(spy3.counts().inspect, 4);

    req.setValuesTyped(new metkit::mars::TypeAny("class"), std::vector<std::string>{"od", "rd"});
    fdb.inspect(req);

    EXPECT_EQUAL(spy1.counts().inspect, 5);
    EXPECT_EQUAL(spy2.counts().inspect, 5);
    EXPECT_EQUAL(spy3.counts().inspect, 5);
...
CASE("lists_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().list, 1);
    EXPECT_EQUAL(spy2.counts().list, 1);
    EXPECT_EQUAL(spy3.counts().list, 1);

    fdb.list(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

    EXPECT_EQUAL(spy1.counts().list, 2);
    EXPECT_EQUAL(spy2.counts().list, 2);
    EXPECT_EQUAL(spy3.counts().list, 2);
    ...
    EXPECT_EQUAL(spy1.counts().list, 3);
    EXPECT_EQUAL(spy2.counts().list, 3);
    EXPECT_EQUAL(spy3.counts().list, 3);

    //// Now match all the rd lanes

    fdb.list(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

    EXPECT_EQUAL(spy1.counts().list, 4);
    EXPECT_EQUAL(spy2.counts().list, 4);
    EXPECT_EQUAL(spy3.counts().list, 4);

    // Explicitly match everything

    fdb.list(fdb5::FDBToolRequest({}, true));

    EXPECT_EQUAL(spy1.counts().list, 5);
    EXPECT_EQUAL(spy2.counts().list, 5);
    EXPECT_EQUAL(spy3.counts().list, 5);
...
CASE("dump_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().dump, 1);
    EXPECT_EQUAL(spy2.counts().dump, 1);
    EXPECT_EQUAL(spy3.counts().dump, 1);

    fdb.dump(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

    EXPECT_EQUAL(spy1.counts().dump, 2);
    EXPECT_EQUAL(spy2.counts().dump, 2);
    EXPECT_EQUAL(spy3.counts().dump, 2);
    ...
    EXPECT_EQUAL(spy1.counts().dump, 3);
    EXPECT_EQUAL(spy2.counts().dump, 3);
    EXPECT_EQUAL(spy3.counts().dump, 3);

    //// Now match all the rd lanes

    fdb.dump(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

    EXPECT_EQUAL(spy1.counts().dump, 4);
    EXPECT_EQUAL(spy2.counts().dump, 4);
    EXPECT_EQUAL(spy3.counts().dump, 4);

    // Explicitly match everything

    fdb.dump(fdb5::FDBToolRequest({}, true));

    EXPECT_EQUAL(spy1.counts().dump, 5);
    EXPECT_EQUAL(spy2.counts().dump, 5);
    EXPECT_EQUAL(spy3.counts().dump, 5);
...
CASE("status_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().status, 1);
    EXPECT_EQUAL(spy2.counts().status, 1);
    EXPECT_EQUAL(spy3.counts().status, 1);

    fdb.status(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

    EXPECT_EQUAL(spy1.counts().status, 2);
    EXPECT_EQUAL(spy2.counts().status, 2);
    EXPECT_EQUAL(spy3.counts().status, 2);
    ...
    EXPECT_EQUAL(spy1.counts().status, 3);
    EXPECT_EQUAL(spy2.counts().status, 3);
    EXPECT_EQUAL(spy3.counts().status, 3);

    //// Now match all the rd lanes

    fdb.status(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

    EXPECT_EQUAL(spy1.counts().status, 4);
    EXPECT_EQUAL(spy2.counts().status, 4);
    EXPECT_EQUAL(spy3.counts().status, 4);

    // Explicitly match everything

    fdb.status(fdb5::FDBToolRequest({}, true));

    EXPECT_EQUAL(spy1.counts().status, 5);
    EXPECT_EQUAL(spy2.counts().status, 5);
    EXPECT_EQUAL(spy3.counts().status, 5);
...
//     EXPECT_EQUAL(spy1.counts().wipe, 1);
//     EXPECT_EQUAL(spy2.counts().wipe, 1);
//     EXPECT_EQUAL(spy3.counts().wipe, 1);

//     fdb.wipe(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

//     EXPECT_EQUAL(spy1.counts().wipe, 2);
//     EXPECT_EQUAL(spy2.counts().wipe, 2);
//     EXPECT_EQUAL(spy3.counts().wipe, 2);
...
//     EXPECT_EQUAL(spy1.counts().wipe, 3);
//     EXPECT_EQUAL(spy2.counts().wipe, 3);
//     EXPECT_EQUAL(spy3.counts().wipe, 3);

//     //// Now match all the rd lanes

//     fdb.wipe(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

//     EXPECT_EQUAL(spy1.counts().wipe, 4);
//     EXPECT_EQUAL(spy2.counts().wipe, 4);
//     EXPECT_EQUAL(spy3.counts().wipe, 4);

//     // Explicitly match everything

//     fdb.wipe(fdb5::FDBToolRequest({}, true));

//     EXPECT_EQUAL(spy1.counts().wipe, 5);
//     EXPECT_EQUAL(spy2.counts().wipe, 5);
//     EXPECT_EQUAL(spy3.counts().wipe, 5);
...
CASE("purge_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().purge, 1);
    EXPECT_EQUAL(spy2.counts().purge, 1);
    EXPECT_EQUAL(spy3.counts().purge, 1);

    fdb.purge(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

    EXPECT_EQUAL(spy1.counts().purge, 2);
    EXPECT_EQUAL(spy2.counts().purge, 2);
    EXPECT_EQUAL(spy3.counts().purge, 2);
    ...
    EXPECT_EQUAL(spy1.counts().purge, 3);
    EXPECT_EQUAL(spy2.counts().purge, 3);
    EXPECT_EQUAL(spy3.counts().purge, 3);

    //// Now match all the rd lanes

    fdb.purge(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

    EXPECT_EQUAL(spy1.counts().purge, 4);
    EXPECT_EQUAL(spy2.counts().purge, 4);
    EXPECT_EQUAL(spy3.counts().purge, 4);

    // Explicitly match everything

    fdb.purge(fdb5::FDBToolRequest({}, true));

    EXPECT_EQUAL(spy1.counts().purge, 5);
    EXPECT_EQUAL(spy2.counts().purge, 5);
    EXPECT_EQUAL(spy3.counts().purge, 5);
...
CASE("stats_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().stats, 1);
    EXPECT_EQUAL(spy2.counts().stats, 1);
    EXPECT_EQUAL(spy3.counts().stats, 1);

    fdb.stats(fdb5::FDBToolRequest::requestsFromString("class=rd,expver=xxxx")[0]);

    EXPECT_EQUAL(spy1.counts().stats, 2);
    EXPECT_EQUAL(spy2.counts().stats, 2);
    EXPECT_EQUAL(spy3.counts().stats, 2);
    ...
    EXPECT_EQUAL(spy1.counts().stats, 3);
    EXPECT_EQUAL(spy2.counts().stats, 3);
    EXPECT_EQUAL(spy3.counts().stats, 3);

    //// Now match all the rd lanes

    fdb.stats(fdb5::FDBToolRequest::requestsFromString("class=rd")[0]);

    EXPECT_EQUAL(spy1.counts().stats, 4);
    EXPECT_EQUAL(spy2.counts().stats, 4);
    EXPECT_EQUAL(spy3.counts().stats, 4);

    // Explicitly match everything

    fdb.stats(fdb5::FDBToolRequest({}, true));

    EXPECT_EQUAL(spy1.counts().stats, 5);
    EXPECT_EQUAL(spy2.counts().stats, 5);
    EXPECT_EQUAL(spy3.counts().stats, 5);
...
CASE("control_distributed_according_to_dist") {
    ...
    EXPECT_EQUAL(spy1.counts().control, 1);
    EXPECT_EQUAL(spy2.counts().control, 1);
    EXPECT_EQUAL(spy3.counts().control, 1);
    ...
    EXPECT_EQUAL(spy1.counts().control, 2);
    EXPECT_EQUAL(spy2.counts().control, 2);
    EXPECT_EQUAL(spy3.counts().control, 2);
    ...
    EXPECT_EQUAL(spy1.counts().control, 3);
    EXPECT_EQUAL(spy2.counts().control, 3);
    EXPECT_EQUAL(spy3.counts().control, 3);
    ...
    EXPECT_EQUAL(spy1.counts().control, 4);
    EXPECT_EQUAL(spy2.counts().control, 4);
    EXPECT_EQUAL(spy3.counts().control, 4);
    ...
    EXPECT_EQUAL(spy1.counts().control, 5);
    EXPECT_EQUAL(spy2.counts().control, 5);
    EXPECT_EQUAL(spy3.counts().control, 5);
