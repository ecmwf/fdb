...
eckit::PathName& tmp_dummy_daos_root() {
    static eckit::PathName d("./wipe_tests_dummy_daos_root");
    return d;
}
#endif

std::string& root_cont_name() {
    static std::string n{"test_wipe_root"};
    return n;
}

int& container_oids_per_alloc() {
    static int n = 1000;
    return n;
}
...
std::string& pool_name() {
    ...
    static std::string pool_name{"fdb_pool_test_wipe"};
#else
    static std::string pool_name = eckit::Resource<std::string>("fdbDaosTestPool;$FDB_DAOS_TEST_POOL", "");
...
CASE("Wipe tests") {
    ...
    SECTION("WIPE SINGLE DATABASE AND SINGLE INDEX") {
        ...
        // fully wipe remaining part of the database
        wipeObject = fdb.wipe(commonReq, true);
        while (wipeObject.next(elem)) {
            std::cout << elem << std::endl;
        }
...
daos:
  ...
  client:
    ...
    std::map<std::string, std::string> configurations{
        {"localDaos", localDaosConfig},
        // localDaosCataloguePosixStore
        // localPosixCatalogueDaosStore
    };
