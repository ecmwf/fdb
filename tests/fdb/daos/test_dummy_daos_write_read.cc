...
CASE( "dummy_daos_write_then_read" ) {
    ...
    int len = daos_array_read(oh, DAOS_TX_NONE, &iod, &sgl, NULL);
    EXPECT(len == (int) array_size);
    ...
    // TODO: need to find a way to rm -rf whole container directory instead. Otherwise, 
    // if a test run fails it will leave garbage for the next run and cause failures
    (pool_path / cont / os.str()).unlink();
    (pool_path / cont / os_kv.str() / key).unlink();
    (pool_path / cont / os_kv.str()).rmdir();
    (pool_path / cont).rmdir();
    pool_path.rmdir();
