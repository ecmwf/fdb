...
struct AutoPool {
    ...
    AutoPool(const pmem::AtomicConstructorBase& constructor) :
        path_(UniquePool().path_), pool_(path_, auto_pool_size, auto_pool_name, constructor) {}
...
public:  // methods
    ...
    PersistentMock(const pmem::AtomicConstructor<T>& ctr) : data_(ctr.size()) { ctr.make(object()); }

    PersistentMock() : ctr_(new pmem::AtomicConstructor0<T>), data_(ctr_->size()) { ctr_->make(object()); }

    template <typename X1>
    PersistentMock(const X1& x1) : ctr_(new pmem::AtomicConstructor1<T, X1>(x1)), data_(ctr_->size()) {
        ctr_->make(object());
    }

    template <typename X1, typename X2>
    PersistentMock(const X1& x1, const X2& x2) :
        ctr_(new pmem::AtomicConstructor2<T, X1, X2>(x1, x2)), data_(ctr_->size()) {
        ctr_->make(object());
    }

    T& object() { return *reinterpret_cast<T*>(&data_[0]); }
