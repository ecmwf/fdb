...
CASE("Step & ClimateDaily - expansion") {
    ...
    std::string tracingID = "test_toKey_tracingID";
    fdb5::Archiver archiver(tracingID, conf);
    auto visitor = fdb5::ArchiveVisitor::create(archiver, key, tracingID, data, 4);
...
CASE("Expver, Time & ClimateDaily - string ctor - expansion") {
    ...
    {
        std::string tracingID = "test_toKey_tracingID";
        fdb5::Archiver archiver(tracingID);
        auto visitor = fdb5::ArchiveVisitor::create(archiver, key, tracingID, data, 4);
        config.schema().expand(key, *visitor);
        fdb5::TypedKey tKey(visitor->rule()->registry());
        tKey.pushFrom(key);
