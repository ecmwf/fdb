...
[[ "$(find . -name '*.index' | wc -l | tr -d '[:space:]')" != "2" ]] && echo "Data not correctly duplicated" && exit -1
...
[[ "$(find . -name '*.index' | wc -l | tr -d '[:space:]')" != "1" ]] && echo "Data should be purged when fully specified" && exit -1
