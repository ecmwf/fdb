...
fdb-list --all --full --minimum-keys="" --porcelain | tee out
[[ "$(wc -l < out | tr -d '[:space:]')" != "24" ]] && echo "FDB modified without --doit" && exit -1
[[ "$(wc -l < wipe_out | tr -d '[:space:]')" != "11" ]] && echo "Unexpected file list for removal" && exit -1 # n.b. includes directory
...
[[ "$(wc -l < wipe_out | tr -d '[:space:]')" != "24" ]] && echo "Unexpected file list for removal" && exit -1
