...
fdb-list --all --full --minimum-keys="" --porcelain | tee out
[[ "$(wc -l < out | tr -d '[:space:]')" != "48" ]] && echo "Data shouldn't change after failing overlay" && exit -1
...
[[ "$(grep "expver=xxxy" out | wc -l | tr -d '[:space:]')" != "24" ]] && echo "Data shouldn't change after failing overlay" && exit -1
[[ "$(grep "expver=xxxy" out | grep "time=1200" | grep "step=3" | wc -l | tr -d '[:space:]')" != "12" ]] && echo "Data shouldn't change after failing overlay" && exit -1
[[ "$(grep "expver=xxxy" out | grep "time=0000" | grep "step=3" | wc -l | tr -d '[:space:]')" != "12" ]] && echo "Data shouldn't change after failing overlay" && exit -1
...
[[ "$(grep "expver=xxxy" out | wc -l | tr -d '[:space:]')" != "36" ]] && echo "Data shouldn't change after failing overlay" && exit -1
