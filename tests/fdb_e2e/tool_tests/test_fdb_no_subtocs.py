...
no_subtoc_combinations = [
    pytest.param(
        ...
        "simple_env_no_subtocs_no_expver_handler",
    ...
    pytest.param(
        ...
        "files_env_no_subtocs_no_expver_handler",
    ...
    pytest.param(
        ...
        "config_yaml_env_no_subtocs_no_expver_handler",
        marks=pytest.mark.yaml,
    ),
    pytest.param(
        "config_json_fdb_setup_no_subtocs_no_expver_handler",
        "config_json_env_no_subtocs_no_expver_handler",
        marks=pytest.mark.json,
    ),
    pytest.param(
        "config_yaml_mars_disks_fdb_setup_no_subtocs_no_expver_handler",
        "config_yaml_mars_disks_env_no_subtocs_no_expver_handler",
        marks=pytest.mark.yaml_mars_disks,
    ),
    pytest.param(
        "config_yaml_tools_fdb_setup_no_subtocs_no_expver_handler",
        "config_yaml_tools_env_no_subtocs_no_expver_handler",
        marks=pytest.mark.yaml_tools,
    ),
    pytest.param(
        "config_json_tools_fdb_setup_no_subtocs_no_expver_handler",
        "config_json_tools_env_no_subtocs_no_expver_handler",
        marks=pytest.mark.json_tools,
    ),
]
...
def test_info(fdb_setup, env_setup, function_tmp, info_no_subtoc_script, request):
    _ = request.getfixturevalue(fdb_setup)
    env = request.getfixturevalue(env_setup)

    run_script(script=info_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
def test_grib2fdb(fdb_setup, env_setup, function_tmp, test_data_path, grib2fdb5_no_subtoc_script, request):
    ...
    print(target_files)
    run_script(script=grib2fdb5_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
def test_list(fdb_setup, env_setup, function_tmp, test_data_path, list_no_subtoc_script, request):
    ...
    # We are skipping the archive as the tests are expecting an empty FDB
    run_script(script=list_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
def test_purge(fdb_setup, env_setup, function_tmp, test_data_path, purge_no_subtoc_script, request):
    ...
    # We are skipping the archive as the tests are expecting an empty FDB
    run_script(script=purge_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
@pytest.mark.root
@pytest.mark.parametrize("fdb_setup, env_setup", no_subtoc_combinations)
def test_root(fdb_setup, env_setup, function_tmp, root_no_subtoc_script, request):
    _ = request.getfixturevalue(fdb_setup)
    env = request.getfixturevalue(env_setup)
    ...
    # We are skipping the archive as the tests are expecting an empty FDB
    run_script(script=root_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
def test_wipe(fdb_setup, env_setup, function_tmp, test_data_path, wipe_no_subtoc_script, request):
    ...
    # We are skipping the archive as the tests are expecting an empty FDB
    run_script(script=wipe_no_subtoc_script, args=None, cwd=function_tmp, env=env)
...
def test_write(fdb_setup, env_setup, function_tmp, test_data_path, write_no_subtoc_script, request):
    ...
    # We are skipping the archive as the tests are expecting an empty FDB
    run_script(script=write_no_subtoc_script, args=None, cwd=function_tmp, env=env)
