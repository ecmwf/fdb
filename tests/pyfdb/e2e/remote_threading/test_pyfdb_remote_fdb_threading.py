    ...
    def _accept_loop(self):
        ...
        while not self.proxy_stop.is_set():
            ...
            self.upstream_thread = threading.Thread(
                target=self._forward, args=(client, upstream), daemon=True
            )
            self.client_thread = threading.Thread(
                target=self._forward, args=(upstream, client), daemon=True
            )
...
def test_remote_fdb_threading_proxy(fdb_servers, data_path):
    ...
    for idx, result in enumerate(results):
        ...
        assert result["list_count"] == 1, (
            f"Worker {idx}: expected 1 listed field, got {result['list_count']}"
        )
        assert result["retrieved"] == expected, (
            f"Worker {idx}: retrieved data does not match x138-300.grib"
        )
    log.debug("all %d worker(s) passed", NUM_WORKERS)


def test_remote_fdb_threading_proxy_partial_read(fdb_servers, data_path):
    """
