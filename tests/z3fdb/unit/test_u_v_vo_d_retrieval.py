...
def test_retrieve_u_and_v_with_short_names(read_only_fdb_setup_for_div_vo_example) -> None:
    """Same as test_retrieve_u_and_v but using MARS short names "u" and "v".

    metkit expands "u" -> "131" and "v" -> "132" before the request reaches
    GribExtractor, so this should behave identically to the numeric-ID variant.
    """
    builder = SimpleStoreBuilder(read_only_fdb_setup_for_div_vo_example)
    builder.add_part(
        {**_BASE_REQUEST, "param": ["u", "v"]},
        [
            AxisDefinition(["date", "time"], Chunking.SINGLE_VALUE),
            AxisDefinition(["param", "levelist"], Chunking.SINGLE_VALUE),
        ],
...
def test_retrieve_vo_and_d_with_short_names(read_only_fdb_setup_for_div_vo_example) -> None:
    """Same as test_retrieve_vo_and_d but using MARS short names instead of numeric paramIds.

    metkit's TypeParam expansion converts "vo" -> "138" and "d" -> "155" before GribExtractor
    sees the request, so the paramId check must not reject these valid textual param names.
    """
    builder = SimpleStoreBuilder(read_only_fdb_setup_for_div_vo_example)
    builder.add_part(
        {**_BASE_REQUEST, "param": ["vo", "d"]},
        [
            AxisDefinition(["date", "time"], Chunking.SINGLE_VALUE),
            AxisDefinition(["param", "levelist"], Chunking.SINGLE_VALUE),
        ],
