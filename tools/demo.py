...
def profile_cmd(args):
    ...
    print(
        f"TOTOAL READ TIME ZFDB: {print_in_closest_unit(fdb_timings[0])} for {len(fdb_timings[1])} fields {args.count} reads"
    )
    print(
        f"TOTOAL READ TIME ZARR-NATIVE: {print_in_closest_unit(zarr_timings[0])} for {len(zarr_timings[1])} fields {args.count} reads"
    )
...
def parse_cli_args():
    ...
    profile_parser.add_argument(
        "-c", "--count", type=int, default=10, help="Count of random reads to perform"
    )
