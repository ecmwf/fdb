...
def register_cmd(subparsers):
    parser = subparsers.add_parser(
        "aggregate-params-sequential",
        help="Aggregate all fields of a native zarr array but will read all fields in sequential order."
        "This simulates random access in a ML context, All fields but random timepoints.",
    )
    parser.set_defaults(func=execute_cmd)
    parser.add_argument(
        "--limit",
        help="Only aggregate up to limit many timepoints",
        type=int,
        default=None,
    )
...
def aggregate(data, limit):
    val = 0
    indices = list(product(*(range(dim) for dim in data.shape[:-2])))
