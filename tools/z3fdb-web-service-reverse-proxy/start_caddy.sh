...
Starts a caddy server in a podman container and mounts the Caddyfile.
The given folder must contain a Caddyfile in the following structure:

```
<path_to_server_data_folder>/caddy/Caddyfile
```

The server runs on port 4430.

If `-c` is specified a default configuration will be placed in the folder specified (the folder
has to be existent). The contained db_store is the root of the set up FDB. Fill it with the 
corresponding data

Options:
  -h, --help       Show this help message and exit
  -c, --create     Create the server_data folder with default values
  -v, --verbose    Enable verbose mode

Examples:
  ${0##*/} <path_to_server_data_folder>
EOF
}

create_server_data() {
    # Directory to check (passed as first argument)
    TARGET_DIR="$INPUT"
    ...
    if [[ -d "$TARGET_DIR" ]]; then
        ...
        # Create Caddyfile with basic content
...
EOF
        ...
        cat << EOF > "$TARGET_DIR/fdb_config.yaml"
engine: toc
schema: $TARGET_DIR/schema
spaces:
- handler: Default
  roots:
  - path: $TARGET_DIR/db_store
type: local
EOF

        echo "FDB config file created: $TARGET_DIR/fdb_config.yaml"

        cat << EOF > "$TARGET_DIR/gribjump_config.yaml"
plugin:
  select: class=(ea|od),stream=(enfo|oper),expver=(00..)
EOF
        echo "GribJump config file created: $TARGET_DIR/gribjump_config.yaml"

        cat << EOF > "$TARGET_DIR/schema"
...
EOF
    ...
    echo "FDB schema file created: $TARGET_DIR/schema"

    mkdir $TARGET_DIR/db_store
    echo "DB store dir created: $TARGET_DIR/db_store"
...
podman run \
--name caddy-server \
-v "$INPUT"/caddy/Caddyfile:/etc/caddy/Caddyfile:Z \
-p 4430:443 \
-it \
--rm \
docker.io/caddy \
