...
from z3fdb.mapping import AxisDefinition, ExtractorType
...
class Request:
    def __init__(self, request: str, axes: list[Tuple[list[str], bool]], extractor_type: str) -> None:
...
class RequestDTO:
    def __init__(self, request: dict[str, Any] | str, axes: list[Tuple[list[str], bool]], extractor_type: ExtractorType) -> None:
...
class RequestDTOMapper:
    ...
    def map_json(cls, json) -> List[Request]:
        request_dtos = [
            RequestDTO(request=request_json["request"], axes=request_json["axes"], extractor_type=ExtractorType[request_json["extractor"]])
